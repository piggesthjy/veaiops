...
const iconMap: Record<string, React.ReactElement> = {
  ...
  IconRobot: <IconRobot />, // Step 7: Chat Bot Management
  ...
  IconList: <IconList />, // Step 12: Content Recognition Rule
  IconClockCircle: <IconClockCircle />, // Step 13: Oncall Change History
};
...
export const SideGuidePanel: React.FC<SideGuidePanelProps> = ({
  ...
  // When the side panel is first opened, if the current step is empty or invalid, activate the first step by default
  // Use ref to record whether initialization has been done, to avoid resetting steps every time the panel opens
  const hasInitializedRef = useRef(false);
  const previousPanelVisibleRef = useRef(false);

  useEffect(() => {
    // Detect state change from closed to open
    const panelJustOpened =
      sideGuidePanelVisible && !previousPanelVisibleRef.current;

    if (panelJustOpened && steps.length > 0 && !hasInitializedRef.current) {
      // Only on first open, if current step is empty or invalid, default to first step
      const firstStep = steps[0];
      // Only default to first step when currentStep is completely invalid (null, undefined, 0, or not in valid step range)
      const isValidStep =
        currentStep && steps.some((s) => s.number === currentStep);
      if (!isValidStep) {
    ...
    // If panel is closed, reset initialization flag so it can be re-initialized next time it opens
    if (!sideGuidePanelVisible && previousPanelVisibleRef.current) {
      hasInitializedRef.current = false;
    }

    // Update panel visibility state
    previousPanelVisibleRef.current = sideGuidePanelVisible;
    ...
    <>
      ...
      <div className={panelClassName}>
        ...
        <Tooltip
          ...
          content="Intelligent Threshold Configuration Wizard"
          position="left"
