# Copyright 2025 Beijing Volcano Engine Technology Co., Ltd. and/or its affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from datetime import timedelta

import pytest_asyncio

from veaiops.schema.documents import Interest
from veaiops.schema.types import EventLevel, InterestActionType, InterestInspectType


@pytest_asyncio.fixture
async def test_interest(test_bot):
    """Create a test interest for oncall rule tests."""
    interest = await Interest(
        bot_id=test_bot.bot_id,
        channel=test_bot.channel,
        name="Test Interest",
        description="Test interest description",
        action_category=InterestActionType.Detect,
        inspect_category=InterestInspectType.RE,
        regular_expression=r"test.*pattern",
        level=EventLevel.P2,
        is_active=True,
        silence_delta=timedelta(seconds=3600),
    ).insert()

    yield interest

    await interest.delete()


@pytest_asyncio.fixture
async def test_semantic_interest(test_bot):
    """Create a test semantic interest for oncall rule tests."""
    interest = await Interest(
        bot_id=test_bot.bot_id,
        channel=test_bot.channel,
        name="Semantic Interest",
        description="Test semantic interest",
        action_category=InterestActionType.Detect,
        inspect_category=InterestInspectType.Semantic,
        examples_positive=["example positive"],
        examples_negative=["example negative"],
        level=EventLevel.P1,
        is_active=True,
        silence_delta=timedelta(seconds=7200),
    ).insert()

    yield interest

    await interest.delete()
